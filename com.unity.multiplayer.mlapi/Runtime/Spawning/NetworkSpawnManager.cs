    ...
    public static class NetworkSpawnManager
    {
        ...
        internal static void ServerDestroySpawnedSceneObjects()
        {
            ...
            foreach (var sobj in spawnedObjects)
            {
                if ((sobj.IsSceneObject != null && sobj.IsSceneObject == true) || sobj.DestroyWithScene)
                {
                    if (CustomDestroyHandlers.ContainsKey(sobj.GlobalObjectIdHash64))
                    {
                        SpawnedObjectsList.Remove(sobj);
                        CustomDestroyHandlers[sobj.GlobalObjectIdHash64](sobj);
        ...
        internal static void DestroyNonSceneObjects()
        {
            ...
            for (int i = 0; i < networkObjects.Length; i++)
            {
                ...
                {
                    if (CustomDestroyHandlers.ContainsKey(networkObjects[i].GlobalObjectIdHash64))
                    {
                        CustomDestroyHandlers[networkObjects[i].GlobalObjectIdHash64](networkObjects[i]);
