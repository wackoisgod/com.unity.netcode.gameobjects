            ...
            if (!ReferenceEquals(m_RpcQueueContainer, null))
            {
                ...
                if (currentFrame != null && currentFrame.IsDirty)
                {
                    ...
                    while (currentQueueItem.QueueItemType != RpcQueueContainer.QueueItemType.None)
                    {
                        ...
                        if (!isTesting)
                        {
                            m_RpcQueueContainer.m_NetworkManager.InvokeRpc(currentQueueItem);
                        }
                ...
                switch (queueItem.QueueItemType)
                {
                    case RpcQueueContainer.QueueItemType.CreateObject:
                    {
                        if (isListening)
                        {
                            ...
                            {
                                m_RpcQueueContainer.m_NetworkManager.InternalMessageSender.Send(clientId, NetworkConstants.ADD_OBJECT, queueItem.NetworkChannel, PoolStream);
                            }
                        }
                    ...
                    case RpcQueueContainer.QueueItemType.DestroyObject:
                    {
                        if (isListening)
                        {
                            foreach (ulong clientId in queueItem.ClientNetworkIds)
                            {
                                m_RpcQueueContainer.m_NetworkManager.InternalMessageSender.Send(clientId, NetworkConstants.DESTROY_OBJECT, queueItem.NetworkChannel, PoolStream);
                            }
                        }
